...
import {
  ...
  CONTEXT_MENU_Y_OFFSET_MULTIPLIER,
} from '../../lib/constants'
...
export function GraphView() {
  ...
  const handleContextMenu = (e: React.MouseEvent) => {
    ...
    if (node?.type === 'class') {
      setContextNode(node)
      setContextMenuPos({
        x: e.clientX - CLASS_LAYOUT_RADIUS,
        y: e.clientY - TOOLTIP_OFFSET * CONTEXT_MENU_Y_OFFSET_MULTIPLIER,
      })
    } else {
      setContextNode(null)
  ...
  return (
    <div ref={containerRef} className="relative h-full w-full">
      ...
      {contextNode && contextMenuPos && (
        <div
          className="border-border bg-card absolute z-50 rounded-md border shadow-lg"
          style={{
